...
Given positions $P$ and $Q$ that are one single move $m$ apart, this means that the inverse position of $Q$
...
$
P arrow^m Q arrow.r.double inv(Q) arrow^m inv(P)
$

Here we use $inv(P)$ to denote the inverse of position $P$ as defined in the #link(<sec:charge-conjugation>)["Charge Conjugation" section].
...
$
"solve"(P, E) = bold(m), #h(3pt) P arrow.r.long^(bold(m)_1) dots.h.c arrow.r.long^(bold(m)_k) E
$
