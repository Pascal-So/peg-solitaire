...
// todo: rotation & mirroring normalizer
...
pub fn bfs(start: Position, end: Position) -> bool {
    let mut map = HashMap::<Position, NodeInfo>::new();
    map.insert(
        start,
        NodeInfo {
            parent: start,
    ...
    queue.push_back((start, Direction::Forwards));
    queue.push_back((end, Direction::Backwards));
            ...
            match dir {
                Direction::Forwards => {
                    if pos.can_jump(jump) {
                        next = Some(pos.apply_jump(jump));
                    }
                }
                Direction::Backwards => {
                    if pos.can_jump_inverse(jump) {
                        next = Some(pos.apply_jump_inverse(jump));
                    }
                }
            }

            if let Some(next) = next {
                if let Some(found) = map.get(&next) {
                    if found.source != current.source {
                        println!(
                            "visited {visited} positions with {attempted} attempts. map size {}",
                            map.len()
                        );
                        return true;
                    }
...
pub fn search(start: Position, end: Position) -> bool {
    let mut visit_map = VisitMap::new();
    ...
    let ok = search_inner(start, end, len, &mut visit_map, &mut state);
...
pub struct State {
    pub explored: u64,
    pub hash_skipped: u64,
    pub path: Vec<(Position, Jump)>,
}
...
pub fn search_inner(
    ...
    for &j in &ALL_JUMPS {
        ...
        if search_inner(next, end, remaining_moves - 1, visit_map, state) {
            // state.path.push((next, j));
            // return true;
        }
    }
    return false;
