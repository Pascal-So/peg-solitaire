...
fn prime_candidates(range: Range<usize>) -> Vec<usize> {
...
fn round_candidates(range: Range<usize>) -> Vec<usize> {
    let mut candidates = vec![];

    'outer: for exponent in 1.. {
        for factor in 4..8 {
            let number = (1usize << exponent) * factor;
...
fn round_minus_one_candidates(range: Range<usize>) -> Vec<usize> {
...
fn get_candidates_groups() -> [(Vec<usize>, String); 3] {
    // 512KB to 42MB
